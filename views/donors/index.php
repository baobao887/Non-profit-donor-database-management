...
<div class="min-h-screen flex">
    ...
    <main class="flex-1 px-6 py-6">
      <header class="flex flex-wrap items-center justify-between gap-4 mb-8">
        <div>
          <p class="eyebrow">Donors / Directory</p>
        ...
        <div class="flex items-center gap-3">
          ...
          <button type="button" id="openAddDonor" class="btn-primary inline-flex items-center gap-2 px-5 py-3 rounded-2xl"><i class="fa-solid fa-plus"></i> Add Donor</button>
        </div>
      </header>
      <section class="grid lg:grid-cols-[1fr_320px] gap-6 mb-8">
        <div class="space-y-6">
          <div class="card-glass p-6">
            ...
            <div class="relative">
              ...
              <input type="search" id="donorSearch" placeholder="Search by name, email, phone..." class="input-glass w-full rounded-3xl pl-12 pr-4 py-4" />
            </div>
          </div>
          <div class="card-glass p-6">
            <div class="flex items-center justify-between mb-4">
              <h3 class="text-lg font-semibold">Refine donor data</h3>
              <button type="button" id="resetFilters" class="text-brand-600 hover:text-brand-700">Reset</button>
            </div>
        ...
        <div class="card-glass p-6">
          ...
          <div class="space-y-4">
            <div class="flex items-center justify-between rounded-3xl bg-ink-100 p-4">
              <div><p class="text-ink-500 text-sm">Silver donors</p><p class="text-xl font-semibold" id="summary-silver">—</p></div>
            </div>
            <div class="flex items-center justify-between rounded-3xl bg-ink-100 p-4">
              <div><p class="text-ink-500 text-sm">Gold donors</p><p class="text-xl font-semibold" id="summary-gold">—</p></div>
            </div>
            <div class="rounded-3xl bg-gradient-to-r from-brand-600 to-brand-800 p-4 text-white shadow-xl">
      ...
      <section class="card-glass p-6 overflow-hidden mb-8">
        <div class="flex flex-wrap items-center justify-between gap-4 mb-5">
          <div><h3 class="text-xl font-semibold">Donor directory</h3><p class="text-ink-500">Live data — add, edit, search, and export.</p></div>
          <span id="showingInfo" class="inline-flex items-center gap-2 px-3 py-2 rounded-full bg-ink-100 text-sm text-ink-600">Showing —</span>
        </div>
  ...
  <div id="donorModal" class="modal">
    ...
    <div class="modal-panel" role="dialog" aria-labelledby="donorModalTitle">
      ...
      <form id="donorForm">
        ...
        <div id="donorFormError" class="mt-4 p-4 bg-danger-50 border border-danger-100 rounded-lg text-danger-700 hidden">
        ...
        <div class="form-field"><label for="donorPhone">Phone</label><input id="donorPhone" class="input-glass" />
          <p class="text-xs text-ink-500 mt-1">At least 7 digits, e.g. 0917 123 4567</p>
        </div>
        ...
        <p class="text-xs text-ink-400 mb-2">Optional — used only for aggregate reporting. Donors may leave these blank.</p>
        <div class="grid grid-cols-2 gap-4">
          <div class="form-field" id="donorLevelField" hidden>
            <label>Level</label>
            <div class="py-2"><span id="donorLevelBadge" class="badge"></span></div>
            <p class="text-xs text-ink-400">Automatically set by total donations.</p>
          </div>
        ...
        <div class="form-field" id="donorRegisteredField" hidden>
          ...
          <p id="donorRegistered" class="py-2 text-ink-600"></p>
        </div>
        <p id="donorLevelHint" class="text-xs text-ink-400 mb-4" hidden>New donors start at Bronze and rank up automatically based on total donations.</p>
        <p id="donorStatusHint" class="text-xs text-ink-400 mb-4" hidden>New donors are added as Active.</p>
